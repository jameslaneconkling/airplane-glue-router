...
module.exports = (db, context) => {
  const Collections = CollectionsFactory(db, context);

  return [
    {
      route: 'collection[{keys:collections}][{ranges:ranges}]',
      get({ collections, ranges }) {
        return Collections.get(collections, ranges)
    ...
    },
    {
      ...
      get({ collections }) {
        return Collections.counts(collections)
    ...
    },
    {
      ...
      get({ collections }) {
        return Collections.getOntologiesList(collections)
          .map(({ collection, list }) => {
            return {
